			...
			shareEndpoint(endpoint.id)
				.then((response) => {
					if (
						...
						console.warn(
							"ShareEndpoint: Unexpected response structure:",
							response
						);
						setError(
							"No valid URL found in response. Please check the server configuration."
						);
					}
				})
				.catch((error) => {
					...
					setError(
						`Failed to generate share link: ${error.message || "Unknown error"}`
					);
					setShareLink(null);
				})
