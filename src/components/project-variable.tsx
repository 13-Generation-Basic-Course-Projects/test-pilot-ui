...
import { useProjectVariableStore } from "@/store/project-variable-slice"; // ❗️ 1. IMPORT THE STORE
...
export default function ProjectVariable({
	...
	// ❗️ 2. GET THE SETTER FUNCTION FROM THE STORE
	const { setProjectVariables } = useProjectVariableStore();

	useEffect(() => {
		loadVariables();
	}, [projectId]);

	// This function now updates both local and global state
	const updateState = (newRows: VariableRow[]) => {
		setRows(newRows);
		setProjectVariables(newRows);
	};

	const loadVariables = async () => {
		try {
			const variables = await getAllProjectVariableAction(projectId, token);
			updateState(variables); // ❗️ 3. UPDATE STATE ON LOAD
			setError(null);
		} catch (err) {
	...
	const handleToggleEnabled = async (index: number) => {
		...
		try {
			...
			const updatedRows = [...rows];
			updatedRows[index].enabled = newEnabled;
			updateState(updatedRows); // ❗️ 3. UPDATE STATE ON TOGGLE
		} catch (err) {
	...
	const handleSaveRow = async (index: number) => {
		...
		try {
			if (row.variableId) {
				...
				setRows(newRows); // Set local rows first before reloading
			}
			await loadVariables(); // Reload to get the latest state from DB and update global store
			setError(null);
			return true;
		} catch (err) {
			console.error("Save operation failed:", err);
			setError(
				`Failed to ${row.variableId ? "update" : "create"} variable: ${
	...
	const handleDeleteRow = async () => {
		...
		try {
			...
			const newRows = rows.filter((_, i) => i !== deleteIndex);
			updateState(newRows); // ❗️ 3. UPDATE STATE ON DELETE
			setDeleteIndex(null);
		} catch (err) {
	...
	const handleToggleAll = async (enabled: boolean) => {
		...
		try {
			...
			const newRows = rows.map((row) => ({ ...row, enabled }));
			updateState(newRows); // ❗️ 3. UPDATE STATE ON TOGGLE ALL
		} catch (err) {
	...
	// --- No changes needed for the local edit/dialog logic below ---
	...
	const handleCellBlur = async (index: number) => {
		...
		if (
			originalRow &&
			(row.variable !== originalRow.variable || row.value !== originalRow.value)
		) {
									...
									<AlertDialog>
										...
										<AlertDialogContent>
											<AlertDialogHeader>
												...
												<AlertDialogDescription>
													This will permanently delete the "{row.variable}"
													variable.
												</AlertDialogDescription>
											</AlertDialogHeader>
											<AlertDialogFooter>
